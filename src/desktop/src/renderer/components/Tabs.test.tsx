...
describe('<Tabs />', () => {
  it('marks the active tab via aria-selected', () => {
    render(<Tabs tabs={TABS} activeTabId="t1" onSelect={vi.fn()} onClose={vi.fn()} />);
  ...
  it('calls onSelect when a tab is clicked', async () => {
    ...
    render(<Tabs tabs={TABS} activeTabId={COMPARE_TAB_ID} onSelect={onSelect} onClose={vi.fn()} />);
  ...
  it('clicking close fires onClose without bubbling to onSelect', async () => {
    const onSelect = vi.fn();
    const onClose = vi.fn();
    render(<Tabs tabs={TABS} activeTabId={COMPARE_TAB_ID} onSelect={onSelect} onClose={onClose} />);
